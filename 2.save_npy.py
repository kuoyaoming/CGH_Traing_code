...
num_threads = 28
feature_size=128
PATH = '/home/power703/work/cgh/data/paper_e3/'
IN_PATH = []
...
def save_3ch_spectrum(wavfile):
    ...
    if len(y)>0:
        ...
        if len(y)/sr > fix_n_sec:
            x = y[:fix_n_sec * sr]
            run=1
        elif 0.1 < len(y)/sr < fix_n_sec :
            z=int((fix_n_sec*sr-len(y))/2)-1
            try:
                x = np.pad(y,(z,z), 'linear_ramp', end_values=(0, 0))
                run=1
            except:
                print("pad error: ",wavfile)
            # ([0]*append_zero)+y+([0]*append_zero)
        elif len(y)/sr < 0.1:
            print('len < 0.1 : ',wavfile)
            run=0
